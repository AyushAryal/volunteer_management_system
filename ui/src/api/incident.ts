...
export let get_incident_list = get_filtered_list<Incident, IncidentFilter>(
  endpoints.incident,
  IncidentDeserializer,
);
export let get_program_list = get_filtered_list<Program, ProgramFilter>(
  endpoints.program,
);
export let get_job_list = get_filtered_list<Job, JobFilter>(
  endpoints.job,
  JobDeserializer,
);
export let get_report_list = get_filtered_list<Report, ReportFilter>(
  endpoints.report,
);
export let get_site_content_list = get_filtered_list<SiteContent, {}>(
  endpoints.site_content,
);
export let get_notification_list = get_filtered_list<Notification, {}>(
  endpoints.notification,
  NotificationDeserializer,
);
export let get_volunteer_geotag_list = get_filtered_list<
  VolunteerGeotag,
  VolunteerGeotagFilter
>(endpoints.volunteer_geotag, VolunteerGeotagDeserializer);

export let get_incident_detail = get_detail<Incident, number>(
  endpoints.incident,
  IncidentDeserializer,
);
export let get_program_detail = get_detail<Program, number>(endpoints.program);
export let get_job_detail = get_detail<Job, number>(
  endpoints.job,
  JobDeserializer,
);
export let get_notification_detail = get_filtered_list<Notification, {}>(
  endpoints.notification,
);

export let get_statistics = get_filtered_endpoint<Statistics, StatisticsFilter>(
...
export async function reset_password_link(email: string): Promise<Response> {
  return token_aware_fetch(endpoints.user_reset_password_link, {
    headers: { "Content-Type": "application/json" },
    method: "POST",
...
export async function reset_password(
  ...
  return token_aware_fetch(`${endpoints.user}/${id}/password_reset`, {
    headers: { "Content-Type": "application/json" },
    method: "POST",
    body: JSON.stringify({
      token,
...
export async function update_volunteer_profile(
  url: string,
  body: BodyInit,
): Promise<Response> {
  let id = get_id(url);
