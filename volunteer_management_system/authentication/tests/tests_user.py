...
class UserTest(TestCase):
    def setUp(self):
        ...
        self.regular_user_email = "regular_user@example.com"
        self.regular_user_password = "123"
    ...
    def test_get_user(self):
        ...
        # Authenticated regular_user
        force_authenticate(request, self.regular_user)
        response = view(request, pk=self.regular_user.pk)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_get_verification_link(self):
        factory = APIRequestFactory()
        view = UserViewSet.as_view({"get": "verification_link"})

        # Unauthenticated
        request = factory.get("/user/verification-link")
        response = view(request)
        self.assertNotEqual(response.status_code, status.HTTP_200_OK)
        ...
        # Authenticated but email is already verified
    ...
    def test_get_verify(self):
        ...
        token = VerificationLinkTokenGenerator().make_token(self.regular_user)
        request = factory.get(f"/user/{self.regular_user.pk}/verify?token={token}")
        response = view(request, pk=self.regular_user.pk)

        # Check if opening link results in a redirect to frontend
        self.assertEqual(response.status_code, status.HTTP_302_FOUND)
        self.assertEqual(
            get_user_model().objects.get(pk=self.regular_user.pk).email_verified, True
        )
    ...
    def test_post_password_reset_link(self):
        ...
        request = factory.post(
            ...
            data={"email": self.regular_user_email},
            format="json",
        )
        response = view(request)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_post_password_reset(self):
        ...
        token = PasswordResetTokenGenerator().make_token(self.regular_user)

        # Incorrect token
        request = factory.post(
            f"/user/{self.regular_user.pk}/password-reset",
            format="json",
            data={
                "token": "invalid",
    ...
    def test_put_change_email(self):
        ...
        request = factory.put(
            f"/user/{self.regular_user.pk}/change_email/", data={}, format="json"
        )
        response = view(request, pk=self.regular_user.pk)
        self.assertNotEqual(response.status_code, status.HTTP_200_OK)
        ...
        request = factory.put(
            f"/user/{self.regular_user.pk}/change_email/", data=data, format="json"
        )
        response = view(request, pk=self.regular_user.pk)
        self.assertNotEqual(response.status_code, status.HTTP_200_OK)
        ...
        request = factory.put(
            f"/user/{self.regular_user.pk}/change_email/",
            data={
                "password": "wrong",
                "new_email": "doesntexist@e.com",
            },
    ...
    def test_put_change_password(self):
        ...
        # Unauthenticated and invalid data
        request = factory.put(
            f"/user/{self.regular_user.pk}/change_password/", data={}, format="json"
        )
        response = view(request, pk=self.regular_user.pk)
        self.assertNotEqual(response.status_code, status.HTTP_200_OK)
        ...
        # Unauthenticated, and valid data
        request = factory.put(
            f"/user/{self.regular_user.pk}/change_password/", data=data, format="json"
        )
        response = view(request, pk=self.regular_user.pk)
        self.assertNotEqual(response.status_code, status.HTTP_200_OK)
        ...
        # Authenticated but wrong password
        request = factory.put(
            f"/user/{self.regular_user.pk}/change_password/",
            data={
                "password": "wrong",
